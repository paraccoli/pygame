...
class MapEditor:
    def __init__(self):
        ...
        # タイルセットの定義
        self.tileset_filenames = [f"Texture/Grand/object{i}.png" for i in range(1, 15)]
        self.tile_sizes = [
    ...
    def draw_map_view(self, screen=None):
        if screen is None:
            screen = self.screen
            ...
            for y in range(self.map_height):
                for x in range(self.map_width):
                    ...
                    if tile_data is not None:
                        if layer == 3:  # 新しいレイヤーの場合
                            pygame.draw.rect(screen, (255, 0, 0), 
                                             (self.PALETTE_WIDTH + x * self.BASE_TILE_SIZE - self.scroll_x,
                                              y * self.BASE_TILE_SIZE - self.scroll_y,
                                              self.BASE_TILE_SIZE, self.BASE_TILE_SIZE))
                        else:
    ...
    def handle_tile_placement(self, mouse_pos, is_left_click):
        ...
        if is_left_click:
            # 左クリック: タイル配置
            if self.current_layer == 3:  # 新しいレイヤーの場合
                self.map_data[self.current_layer][map_y][map_x] = (255, 0, 0)  # 赤く塗りつぶす
            else:
    ...
    def load_map(self):
        ...
        if file_path:
            ...
            # レイヤー4がない場合は追加
            if len(self.map_data) < self.layers:
                for _ in range(self.layers - len(self.map_data)):
                    self.map_data.append([[None for _ in range(self.map_width)] for _ in range(self.map_height)])
            print(f"Map loaded from {file_path}")

    def show_map_in_new_window(self):
        """新しいウィンドウにマップを表示"""
        new_window = pygame.display.set_mode((1920, 1080))
